...
serialize_tree_with_features <- function(lines, root_idx) {
  line <- lines[root_idx]
  ...
  left_child <- serialize_tree_with_features(lines, left_idx)
  right_child <- serialize_tree_with_features(lines, right_idx)
...
for (settings_line in settings_lines) {
  ...
  start_time <- Sys.time()
  tree <- ctree(Surv(time, event) ~ ., data = sdata)
  end_time <- Sys.time()
  ...
  tree_string <- serialize_tree_with_features(tree_lines, 1)
